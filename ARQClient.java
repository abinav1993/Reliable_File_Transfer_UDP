...
public class ARQClient {
	...
	private long[] timer;
	public ARQClient(InetAddress serverIP, int serverport, int n, int mss, String filepath){
		this.serverIP = serverIP;
		this.serverport = serverport;
		this.n = n;
		this.mss = mss;
		this.path =filepath;
		this.timer = new long[n];
...
//		File f = new File(args[2]);
		...
		ARQClient sender = new ARQClient(IPAddress, port, n, mss, path);
		System.out.println(sender.transferFile());
	}
...
//				System.out.println();
				...
				client.send(p);
//				System.out.println("sending packet "+lastSent);
				timer[lastSent % n] = System.currentTimeMillis();
				// withOutAck.add(p);
				lastSent++;
			}
			try {
...
//				System.out.println("Ack reveived for packet:"+AckedSeqNum);
				...
				// }
				long time_waiting = System.currentTimeMillis() - timer[NoAck % n];
				if(time_waiting > 1000){
					DatagramPacket p = rdt_send(data, mss, NoAck, serverIP, serverport);
					client.setSoTimeout(1000);
					client.send(p);
					System.out.println("sending packet "+NoAck);
					timer[NoAck % n] = System.currentTimeMillis();
				}else{
