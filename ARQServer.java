...
public class ARQServer {
	...
	private int window;
	private byte buffer[][];
	public ARQServer(int port, String fname, float p, int window){
		this.port = port;
		this.fname = fname;
		this.p = p;	
		this.window = window;
...
//				System.out.println();
				if (checkSumVerify == sender_checksum) {
					// System.out.println("Checksum verified");
						if(buffer[seqNum % window] == null){
							buffer[seqNum % window] = data;
						}
						while(buffer[waitingFor % window] != null){
							fos.write(buffer[waitingFor % window]);
							buffer[waitingFor % window] = null;
							waitingFor++;
						}
...
//						System.out.println("Sending Ack for seqNum:"+waitingFor);
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length != 4) {
			System.out.println("Format: <Server port> <Filename> <Probability> <window-size>");
			return;
		}
		...
		ARQServer receiver = new ARQServer(port, filename, p, window);
